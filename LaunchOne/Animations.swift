...
enum LNAnimations {
    // MARK: - Springs - performance-tuned animation presets
    static var springFast: Animation {
        guard AnimationPreferences.isEnabled else {
            return .linear(duration: 0.0001)
        }
        return .spring(
            response: AnimationPreferences.springResponse,
            dampingFraction: 0.8
        )
    }

    // MARK: - Performance-tuned animations
