...
struct AppInfo: Identifiable, Equatable, Hashable {
    ...
    static func from(
        ...
        let fallbackName = normalizeCandidate(
            url.deletingPathExtension().lastPathComponent
        )
        let bundle = Bundle(url: url)
        let localizedName = localizedAppName(
            for: url,
            preferredName: preferredName,
            fallbackName: fallbackName,
            bundle: bundle
        )
        let englishName = englishAppName(
            preferredName: preferredName,
            fallbackName: fallbackName,
            bundle: bundle
        )
    ...
    // MARK: - Resolve localized application name
    private static func localizedAppName(
        ...
        if let bundle {
            consider(
                bundlePreferredDisplayName(bundle),
                source: "BundlePreferredDisplayName"
            )
        }
    ...
    private static func englishAppName(
        preferredName: String?,
        fallbackName: String,
        bundle: Bundle?
    ) -> String {
    ...
    {
        let preferredLocales = Bundle.preferredLocalizations(
        ...
        if let chosen = preferredLocales.first,
            ...
            let stringsPath = localizedBundle.path(
                forResource: "InfoPlist",
                ofType: "strings"
            ),
            let dict = NSDictionary(contentsOfFile: stringsPath)
                as? [String: String]
        {
            if let displayName = dict["CFBundleDisplayName"],
                !displayName.isEmpty
            {
                return displayName
            }
        ...
        if let localizedInfo = bundle.localizedInfoDictionary {
            if let displayName = localizedInfo["CFBundleDisplayName"]
    ...
    private static func userPreferredLanguages() -> [String] {
        if let languages = UserDefaults.standard.array(forKey: "AppleLanguages")
