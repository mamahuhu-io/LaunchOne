...
struct GeometryUtils {
    static func cellOrigin(
        ...
        let y =
            gridPadding + CGFloat(row) * (appHeight + rowSpacing)
            - scrollOffsetY

        // For multi-page layouts
        let effectivePageOffsetX =
            CGFloat(pageIndex - currentPage)
            * (containerSize.width + pageSpacing)

        return CGPoint(x: x + effectivePageOffsetX, y: y)
    }

    static func cellCenter(
        for index: Int,
        containerSize: CGSize,
        pageIndex: Int,
        columnWidth: CGFloat,
    ...
    static func indexAt(
        ...
        let col = Int(
            (localX + columnSpacing / 2) / (columnWidth + columnSpacing)
        )
        ...
        guard offsetInPage >= 0 && offsetInPage < itemsPerPage else {
            return nil
        }
