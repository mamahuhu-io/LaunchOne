        ...
        VStack(spacing: 8) {
            ZStack {
                let renderedIcon: NSImage = {
                    switch item {
                    case .app(let app):
                        if let cachedIcon = AppCacheManager.shared
                            .getCachedIcon(for: app.url.path),
                ...
                if isFolderIcon {
                    RoundedRectangle(cornerRadius: iconSize * 0.18)
                        ...
                        .liquidGlass(
                            in: RoundedRectangle(cornerRadius: iconSize * 0.18)
                        )
                        .shadow(
...
private struct PressFeedbackButtonStyle: ButtonStyle {
    ...
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .animation(
                LNAnimations.springFast,
                value: configuration.isPressed && enabled
                    && shouldAllowPressFeedback
            )
    }
