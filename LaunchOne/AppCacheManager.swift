...
final class AppCacheManager: ObservableObject {
    ...
    private var iconCacheOrder: [String] = []  // Mutable array to implement real LRU
    ...
    /// Generate app caches - call at app startup or after scanning
    func generateCache(
        from apps: [AppInfo],
        items: [LaunchpadItem],
        itemsPerPage: Int,
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            // Collect all apps to cache, including those inside folders
            var allApps: [AppInfo] = []
            allApps.append(contentsOf: apps)

            // Extract apps inside folders from items
            for item in items {
            ...
            // Deduplicate to avoid caching the same app twice
    ...
    /// Smart preloading: preload icons for the current and adjacent pages
    func smartPreloadIcons(
        for items: [LaunchpadItem],
        currentPage: Int,
        itemsPerPage: Int
    ) {
    ...
    func refreshCache(
        ...
        // Extract apps in folders from items
        for item in items {
            if case .folder(let folder) = item {
                allApps.append(contentsOf: folder.apps)
            }
    ...
    private func cacheGridLayout(
        _ items: [LaunchpadItem],
        itemsPerPage: Int,
        columns: Int,
        rows: Int
    ) {
        // Cache computed data related to grid layout
        let layoutData = GridLayoutCacheData(
            totalItems: items.count,
            itemsPerPage: itemsPerPage,
            columns: columns,
            rows: rows,
            pageCount: (items.count + max(itemsPerPage, 1) - 1)
                / max(itemsPerPage, 1)
        )
    ...
    private func calculatePageInfo(
        for items: [LaunchpadItem],
        itemsPerPage: Int
    ) -> [PageInfo] {
        let sanitizedItemsPerPage = max(itemsPerPage, 1)
        let pageCount =
            (items.count + sanitizedItemsPerPage - 1) / sanitizedItemsPerPage
        ...
        for pageIndex in 0..<pageCount {
            ...
            let appCount = pageItems.filter {
                if case .app = $0 { return true } else { return false }
            }.count
            let folderCount = pageItems.filter {
                if case .folder = $0 { return true } else { return false }
            }.count
            let emptyCount = pageItems.filter {
